...
import { getCollection, getCollectionItems } from '@/lib/collections'
...
export async function POST(_req: NextRequest, { params }: Ctx) {
  ...
  const collection = await getCollection(id, ownerId)
  if (!collection) return Response.json({ error: 'not_found' }, { status: 404 })

  const items = await getCollectionItems(id)
  if (items.length === 0) return Response.json({ error: 'collection is empty' }, { status: 422 })

  // Fetch full content for each item (join all chunks)
  ...
  await sql`UPDATE collections SET published = TRUE, updated_at = NOW() WHERE id = ${id}`
