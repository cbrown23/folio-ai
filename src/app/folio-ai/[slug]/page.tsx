...
async function fetchIntroExcerpt(ownerId: string): Promise<string> {
  try {
    // Prefer the compiled folio document (generated by Apply to folio); fall back to raw bio
    const rows = await sql`
      SELECT content, type FROM documents
      WHERE owner_id = ${ownerId} AND type IN ('folio', 'bio')
      ORDER BY CASE type WHEN 'folio' THEN 0 ELSE 1 END, created_at DESC
      LIMIT 1
    `
